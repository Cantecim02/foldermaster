...
import { convertUploadedImagesToPdf, convertUploadedMediaFile } from "./mediaDownloaderApi";
...
async function pdfToImages(file: AppFile, outputType: FileType): Promise<ConvertedFile[]> {
  ...
  const documentTask = pdfjs.getDocument({
    data: new Uint8Array(await readWebArrayBuffer(file.uri)),
    disableWorker: true
...
function validateFiles(files: AppFile[], inputType: FileType) {
  for (const file of files) {
    ...
    const normalizedMimeType = fileTypeFromMime(file.mimeType);
    if (normalizedExtension !== inputType && normalizedMimeType !== inputType) {
      throw new Error("ERR_TYPE_MISMATCH");
    }
  }
}

function fileTypeFromMime(mimeType?: string | null): FileType | null {
  ...
  if (normalized === "application/vnd.uyap.udf" || normalized === "application/xml" || normalized === "text/xml") {
    return "udf";
  }
  return null;
}
...
async function xlsxToCsv(file: AppFile): Promise<ConvertedFile> {
  ...
  const workbook =
    Platform.OS === "web"
      ? XLSX.read(await readWebArrayBuffer(file.uri), { type: "array" })
      : XLSX.read(
          await FileSystem.readAsStringAsync(file.uri, {
            encoding: FileSystem.EncodingType.Base64
          }),
...
async function csvToXlsx(file: AppFile): Promise<ConvertedFile> {
  ...
  const outputBase64 = XLSX.write(workbook, { bookType: "xlsx", type: "base64" });
  return writeBase64Output(file.name, "xlsx", outputBase64);
}
